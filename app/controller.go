...
func startController(c *cli.Context) error {
	...
	go func(c *controller.Controller) {
		for <-startAutoSnapDeletion {
			go func() {
				if err := autoDeleteSnapshot(c); err != nil {
					logrus.Error(err)
				}
			}()
		}
...
func listSnapshots(c *controller.Controller, replicas []rest.Replica) ([]string, error) {
	sorted := true
	snapshots, latest, err := getCommonSnapshots(replicas, sorted)
	if err != nil {
		return nil, err
	}
	// ignore latest snapshot and empty snapshot
	// returned from getCommonSnapshots
	snaps := make([]string, len(snapshots)-2)
	i := 0
	for _, s := range snapshots {
		// donot add latest snapshot
		if latest == s || s == "" {
			continue
		}
		snaps[i] = strings.TrimSuffix(strings.TrimPrefix(s, "volume-snap-"), ".img")
		i++
	}
